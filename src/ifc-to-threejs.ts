...
function typeColorRGB(name: string | undefined): [number, number, number] {
  const t = name || 'IfcProduct';
  let h = 0;
  for (let i = 0; i < t.length; i++) h = (h * 31 + t.charCodeAt(i)) & 0xffff;
  const col = new THREE.Color();
  col.setHSL((h % 360) / 360, 0.55, 0.55);
  ...
  for (const [, meshes] of colorBuckets) {
    ...
    for (const m of meshes) {
      ...
      for (let i = 0; i < m.positions.length; i += 3) {
        positions[posOffset + i]     = m.positions[i]     + ox;
        positions[posOffset + i + 1] = m.positions[i + 1] + oy;
        positions[posOffset + i + 2] = m.positions[i + 2] + oz;
      }
...
function mergeWithVertexColors(
  ...
  const classColors = new Float32Array(totalVertices * 3);
  const indices = new Uint32Array(totalIndices);
  ...
  for (const m of meshes) {
    ...
    for (let i = 0; i < m.positions.length; i += 3) {
      positions[base + i]     = m.positions[i]     + ox;
      positions[base + i + 1] = m.positions[i + 1] + oy;
      positions[base + i + 2] = m.positions[i + 2] + oz;
    }
    normals.set(m.normals, vOffset * 3);

    const [r, g, b] = m.color;
    const [cr, cg, cb] = typeColorRGB(m.ifcType);
    for (let v = 0; v < vertCount; v++) {
      colors[(vOffset + v) * 3 + 0] = r;
      colors[(vOffset + v) * 3 + 1] = g;
      colors[(vOffset + v) * 3 + 2] = b;
      classColors[(vOffset + v) * 3 + 0] = cr;
      classColors[(vOffset + v) * 3 + 1] = cg;
      classColors[(vOffset + v) * 3 + 2] = cb;
    }
  ...
  mesh.userData.classColors = classColors;
